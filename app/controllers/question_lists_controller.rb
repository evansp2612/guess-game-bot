class QuestionListsController < ApplicationController
  ...
  def create
    answers = params[:question_list][:answer].reject { |a| a.empty? }.uniq
    if answers.empty? or params[:question_list][:question].blank?
      render plain: "Question and Answers can't be empty", status: :unprocessable_entity
    else
      question = QuestionList.create(question: params[:question_list][:question], answer: answers)
      render plain: question.errors.full_messages[0] || "Question has been submitted to be approved", status: :ok
    end
  ...
  private
    ...
    def question_list_params
      params.require(:question_list).permit(:question, :enabled, :answer => [])
    end
end
